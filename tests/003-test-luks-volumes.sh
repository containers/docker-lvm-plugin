...
test_luks_volumes(){
  ...
  docker volume create -d lvm --opt size=0.2G --opt keyfile=$TEMP_DIR/key.bin --name crypt_vol 2>&1 >/dev/null
  if [ $? -ne 0 ]; then
    echo "ERROR: $testname: Error creating luks volume crypt_vol."
    return $test_status
  fi

  docker volume inspect crypt_vol 2>&1 >/dev/null
  if [ $? -ne 0 ]; then
    echo "ERROR: $testname: Error inspecting volume crypt_vol."
    return $test_status
  fi

  docker volume create -d lvm --opt size=0.2G --opt snapshot=crypt_vol --name crypt_vol_snapshot 2>&1 >/dev/null
  if [ $? -ne 0 ]; then
    echo "ERROR: $testname: Error creating snapshot of volume crypt_vol."
    return $test_status
  fi

  docker volume inspect crypt_vol_snapshot 2>&1 >/dev/null
  if [ $? -ne 0 ]; then
    echo "ERROR: $testname: Error inspecting volume crypt_vol_snapshot."
    return $test_status
  fi

  docker volume rm crypt_vol_snapshot 2>&1 >/dev/null
  if [ $? -ne 0 ]; then
    echo "ERROR: $testname: Error removing volume crypt_vol_snapshot."
    return $test_status
  fi

  docker volume rm crypt_vol 2>&1 >/dev/null
  if [ $? -ne 0 ]; then
